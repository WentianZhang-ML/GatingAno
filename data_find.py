...
def IDRiD_hog_process(data_dir, level, label_save_dir):
    ...
    for i, file in enumerate(path_list):
        ...
        if '/test/abnormal/' in file:
            ...
            dict['image_hog_path'] = file_name +'_4_'+num+'.png'
            if level == 'image':
                dict['image_normal_or_abormal_label'] = 1
            if level == 'pixel':
    ...
    for file in path_list:
        ...
        for j in range(3):
            for i in range (3):
                ...
                flag = flag+1
def ADAM_hog_process(data_dir, level, label_save_dir):
    if not os.path.exists(label_save_dir):
        os.makedirs(label_save_dir)
    train_json = []
    test_json = []
...
if __name__=="__main__":
    ...
    if 'IDRiD' in data_dir:
        ...
        if 'IDRiDc' in data_dir:
            IDRiD_hog_process(data_dir, 'image', './labels/IDRiDc/')
        else:
            IDRiD_hog_process(data_dir, 'pixel', './labels/IDRiD/')

    if 'ADAM' in data_dir:
        ADAM_hog_generate(data_dir)
        hog_split(data_dir,'ADAM')
        if 'ADAMc' in data_dir:
            ADAM_hog_process(data_dir, 'image', './labels/ADAMc/')
        else:
            ADAM_hog_process(data_dir, 'pixel', './labels/ADAM/')
